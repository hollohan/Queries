...
class Queries():
	def __init__(self, db='log', host='localhost', user='grower', password='grower'):
	...
	def run_query(self, q_string):
		...
		while True:
			try:
				...
				print('[Queries.py][run_)query()] ERROR->{}'.format(e))
				reconnect()
	...
	def delete (self, frm, whr=0):
		q_string = 'DELETE FROM {}'.format(frm)
	...
	def insert(self, table, values):
		...
		r = self.run_query('SELECT LAST_INSERT_ID()')
		return (r[0][0])
